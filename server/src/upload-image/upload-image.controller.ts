...
export class ImageController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Upload multiple images to AWS S3 with dynamic sizes',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    description: 'Upload image files and sizes',
  ...
  async uploadImages(
    ...
    return Promise.all(
      files.map((file) => this.imageService.uploadImageToS3(file, sizes)),
    );
  }
}
